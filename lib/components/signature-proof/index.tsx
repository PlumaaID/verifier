...
const SignatureProof: FC<SignatureProofProps> = ({ proof, ...props }) => {
  ...
  }, [certificate.publicKey, proof.hash, proof.signature]);
    ...
    <ThemeProvider storageKey="plumaa-id-verifier-theme">
      <Card className={cn("my-2", props.className)} {...props}>
        <CardHeader>
          ...
          <CardDescription>
            En conformidad con el Artículo 97 del Código de Comercio se
            desglosan los elementos necesarios para cumplir con los requisitos
        ...
        <CardContent className="flex justify-between items-center py-4">
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                <p className="text-sm">
                  La firma electrónica es válida y e implica que la llave
                  privada se utilizó para firmar el digestivo del documento en
                  la prueba.
                </p>
              </TooltipContent>
            </Tooltip>
        ...
        <CardContent className="mt-3">
          ...
          <p className="text-sm text-muted-foreground mb-3">
            El protocolo OCSP es un mecanismo que verifica si el certificado no
            ha sido revocado. La respuesta es firmada por el emisor del
            certificado y contiene el estado actual del certificado (e.g. SAT).
            Así se garantiza el no repudio de la firma.
          </p>
